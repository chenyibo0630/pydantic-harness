...
def build_system_prompt(
    ...
    appending the skills section. Called per **new** conversation (not per
    turn), so the contents stay stable for the conversation's lifetime."""
...
def _session_instructions(ctx: RunContext[MemoryDeps]) -> str:
...
def create_agent(
    ...
    flows in per-request via ``MemoryDeps.system_prompt`` (resolved by the
