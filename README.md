...
每次 `set()` 都扫描**所有** `ToolReturnPart`,大于 `min_size`(默认 2048 字符)的把 `content` 搬到缓存,原位换成占位符:
...
- `EvictingConversation`:`min_size=2048`(小于此值的 tool result 留原文 inline,模型 turn N+1 直接看见,不用 recall。参考 openclaw 的 4K soft-trim 阈值定的折中)
